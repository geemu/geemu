...
public class LoginServiceImpl implements LoginService {
    ...
    public UserEntity login(NamePwdReq condition) {
        ...
        if (null == userEntity) {
            throw new BizException(BaseResponseState.USER_NOT_EXISTED, "用户名或密码错误");
        }
        if (!userEntity.getPassword().equals(condition.getPassword())) {
            throw new BizException(BaseResponseState.INVALID_PASSWORD, "用户名或密码错误");
        }
        if (!userEntity.getEnabled()) {
            throw new BizException(BaseResponseState.USER_LOCKED);
        }
        log.info("用户:{}登录成功", userEntity);
        return userEntity;
    }
